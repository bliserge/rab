<template>
  <v-app>
    <v-navigation-drawer v-model="drawer" app style="backgroupnd: #3f4d67">
      <v-list-item>
        <v-list-item-content>
          <v-list-item-title class="text-h6"> Post Harvest </v-list-item-title>
            ...
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                <v-avatar class="avatar" v-bind="attrs" v-on="on">
                  <img src="~assets/profile.png" alt="mg" />
                </v-avatar>
              </v-badge>
            </template>
            <v-list>
              <v-list-item>
                <v-list-item-content>
                  <v-list-item-subtitle> welcome Again </v-list-item-subtitle>
                  <v-list-item-title>
                    {{ names }}
                  </v-list-item-title>
                </v-list-item-content>
...
export default {
  data: () => ({
    ...
    searchKeyWord: '',
    names: 'Serge Bliss',
    items: [['mdi-home-outline', 'Dashboard', '/home']],
    profile_items: [],
  }),
  methods: {
    logout() {
...
.input-field >>> .v-input__slot::before,
.input-field {
  border-color: #47f6aa;
  width: 300px;
}
